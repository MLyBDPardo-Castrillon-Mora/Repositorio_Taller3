...
db <- read.csv("C:/BigDataLaura/Repositorio_Taller3/Repositorio_Taller3/Script/train.csv")
kaggle <- read.csv("C:/BigDataLaura/Repositorio_Taller3/Repositorio_Taller3/Script/test.csv")
...
# Ver base de datos
glimpse(db)
stargazer(db$property_type)
prop.table (table(db$property_type))
prop.table (table(db$operation_type))
#summary(db$price)
...
# Encontramos variables a partir del campo desription

db<-mutate(db, iluminado = ifelse(grepl("iluminado", db$description), "Si", "No"))
prop.table (table(db$iluminado))

db<-mutate(db, duplex = ifelse(grepl("duplex", db$description), "Si", "No"))
prop.table (table(db$multifamiliar))
