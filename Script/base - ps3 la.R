...
db <- read.csv("C:/00 -- Laura/train.csv")
kaggle <- read.csv("C:/00 -- Laura/test.csv")
...
tm_geometria_1=st-distance(tm_geometria)
k3 <- kmeans(tm_geometria_1, centers = 3, nstart = 25)
k3$cluster
str(k3)

tm_geometria_1<- tm_geometria_1 %>% mutate(clusters=factor(k3$cluster))


ggplot() +
  geom_sf(data=tm_geometria_1,aes(col=tm_geometria_1$clusters)) + #graficamos las predicciones
  theme_bw()
...
db$area_number <- ifelse(db$area_number < 45, NA, db$area_number) # discutir si se incluye o no se incluye
...
model.3.1 <- randomForest (price ~ bedrooms + dmin_bar + dmin_gym + dmin_hosp + 
                             + dmin_malls + dmin_park + dmin_rest + dmin_school 
                           + dmin_tm + dmin_casino  + dmin_policia
                           + garaje + deposito + balcon + ascensor + casa 
                           + planta + seg + conjunto + vista + chimenea 
                           + smart, data = db, ntree = 100)

m3.1 <- predict(model.3.1)
mae_3.1 <- mean(abs(db$price - m3.1))
mae_3.1
...
model.4.1 <- randomForest (price ~ bedrooms + dmin_bar + dmin_gym + dmin_hosp + 
                             I(dmin_hosp^2) + dmin_malls + dmin_park + dmin_rest 
                           + dmin_school + dmin_tm + I(dmin_tm^2) + dmin_casino 
                           + dmin_disco + dmin_policia + I(dmin_policia^2)
                           + garaje + deposito + balcon + ascensor + casa 
                           + planta + seg + conjunto + vista + chimenea 
                           + smart, data = db, ntree = 500)

m4.1 <- predict(model.4.1)
head(m4.1)
mae_4.1 <- mean(abs(db$price - m4.1))
mae_4.1

p_load(MLmetrics)
m6 <- predict(model.4.1)
scales::dollar(MAE(y_pred = m6, y_true = db$price))
scales::dollar(RMSE(y_pred = m6, y_true = db$price))


#mse_4 <- mean(sum(db$price - m4)^2)
#mse_4
...
model.5 <- gbm(price ~ bedrooms + dmin_bar + dmin_gym + dmin_hosp + 
               ...
               + garaje + deposito + balcon + ascensor + casa 
               + planta + seg + conjunto + vista + chimenea 
               + smart, data = db, n.trees = 500, interaction.depth = 3,
               shrinkage = 0.33, distribution = "gaussian")
...
# correlacion de variables del modelo y exporta a Latex

x1 <- db  %>% select(bedrooms, dmin_hosp, dmin_malls, 
                     dmin_park, dmin_school, dmin_tm, dmin_policia, garaje, deposito, balcon,
                     ascensor, smart)
...
library(xtable)
#db_prueba <-  db[,1:3]
#db_prueba <-  db_prueba[1:5,]
#db_prueba
print(xtable(data_cor), include.rownames = FALSE)
...
db <- db %>% mutate(logprice=log(price))
set.seed (10000)

inTrain <- createDataPartition(y=db$logprice,p=0.7,list=FALSE)
train <-db[inTrain,]
test <-db[-inTrain,]
colnames(train)
y <- train$logprice
x <- train  %>% select(bedrooms, dmin_hosp, dmin_malls, 
                       dmin_park, dmin_rest, dmin_school, dmin_tm, garaje, deposito, balcon,
                       ascensor, casa, seg, conjunto, vista)
sl.lib <- c("SL.randomForest", "SL.lm")

fit_Y <- SuperLearner(Y = y,  X= data.frame(x), method = "method.NNLS", SL.library = sl.lib)
fit_y

test <- test %>% mutate (yhat_Sup=predict(fit_Y, newdata=data.frame(test),onlySL=T)$pred)
head(test$yhat_Sup)
with(test,mean(abs(logprice-yhat_Sup)))     #MAE
with(test,mean(sum(logprice-yhat_Sup)^2))     #MSE
...
#write.csv(export, "prueba_4x.csv", row.names = FALSE)


export <- as.data.frame(kaggle$property_id)
export$price <- predict(model.4.1 , newdata = kaggle)
colnames(export)[1] <- "property_id"
write.csv(export, "prueba_5.csv", row.names = FALSE)
